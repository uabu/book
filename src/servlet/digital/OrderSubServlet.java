...
public class OrderSubServlet extends HttpServlet {
	...
	private static final String CART_PATH="jsp/digital/cart.jsp" ;
	private static final String ORDER_PAY_PATH="jsp/digital/ordersuccess.jsp";
	...
	private void orderShip2(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		String orderId=request.getParameter("id");
		...
		if(orderDao.orderStatus(Integer.parseInt(orderId),status)) {
			request.setAttribute("orderMessage", "一个订单操作成功");
		}else {
			request.setAttribute("orderMessage", "一个订单操作失败");
		}
	...
	private void myOrderList(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		if(user==null) {
			response.sendRedirect("jsp/digital/reg.jsp?type=login");
		}else {
			OrderDao orderDao=new OrderDaoImpl();
			...
			for(Order order:orderList) {
				...
				for(OrderItem oi:order.getoItem()) {
					//通过商品id获取商品对象
					oi.setDigital(digitalDao.findDigitalById(oi.getDigitalId()));
				}
			...
			request.getRequestDispatcher("jsp/digital/myorderlist.jsp").forward(request, response);
