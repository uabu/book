...
<script type="text/javascript" src="js/digital/user_reg_login.js"></script>
<style type="text/css">
.wrapper {
	minheight: 400px;
}
								...
								<c:forEach items="${shopCart.map}" var="i">
									<tr class="row" id="pro-tr-${i.key}">
										<td><img class="img-responsive col-md-4"
											src="${i.value.digital.upLoadImg.imgSrc }" alt="" />
											<div class="col-md-8" style="padding:50px 0 50px 0;">
												<a href="digitaldetail?digitalId=${i.key}">${i.value.digital.digitalName}</a>
												<%-- <p>${i.value.digital.author}</p>
												<p>${i.value.digital.press}</p> --%>
											</div>
										</td>
										<td style="padding:50px 0 50px 0;">￥<i>${i.value.digital.price}</i></td>
										...
										<img class="icon" src="images/digital/close-icon.png" alt="" style="margin:50px 0 50px 0;width:15px"></a></td>
									</tr>
								</c:forEach>
								<tr class="row">
	  ...
	  //输入框处理
		...
		function changeQ(obj,id,val){
			$.ajax({
				...
				dataType:"json",
				async:true,
				data:{"digitalId":id,"quantity":val},
				type:"POST",
		...
		$("#tosettle").click(function(){
			$.get("UserServlet?action=landstatus",function(data){
				if(data.status=="y"){
					window.location.href="${basePath}jsp/digital/conorder.jsp";
				}else{
					$("#myModal").modal("show");
				}
			},"json")
