					...
					<c:when test="${!empty orderList }">
						<c:forEach items="${orderList}" var="order" varStatus="varstatus">
							...
							<c:forEach items="${order.oItem}" var="oit" varStatus="vs">
								<tr>
									<td>
										<img class="img-responsive col-md-2" src="${oit.digital.upLoadImg.imgSrc}" alt="" />
										<span class="col-md-10">${oit.digital.digitalName}</span>
									</td>
									<td>￥${oit.digital.price}</td>
									<td>${oit.quantity}</td>
									<td><a href="OrderServlet?action=ship&id=${order.orderId}&status=40">申请退货</a></td>
									<c:choose>
										<c:when test="${vs.first}">
											...
											<td style="border:0;border-left:1px solid #ddd;">
												...
												<c:if test="${order.orderStatus eq 2}">
													<p>已发货</p>
													<a href="OrderServlet?action=ship&id=${oit.orderId}" class="btn btn-sm btn-success" onclick="confirm('确认收货了么？')">确认收货</a>
												</c:if>
												<c:if test="${order.orderStatus eq 3}">订单已完成</c:if>
												<c:if test="${order.orderStatus eq 40}">退货待审批</c:if>
												<c:if test="${order.orderStatus eq 41}">退货审批通过</c:if>
												<c:if test="${order.orderStatus eq 42}">拒绝退货</c:if>
											</td>
										</c:when>
										<c:otherwise>
