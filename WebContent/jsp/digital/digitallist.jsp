			...
			<div class="main container">
				<div class="row">
					<div class="col-md-2 main-left">
						<h3>商品分类</h3>
						<ul id="catalog-list">
							<li><a href="DigitalList">所有商品</a></li>
								...
								<ul class="row"> 
									<c:choose>
										<c:when test="${!empty digitalList}">
											<c:forEach items="${digitalList}" var="i">
												<li class="col-md-3">
													<div class="list">
														<a href="digitaldetail?digitalId=${i.digitalId}">
															<img class="img-responsive" src="${i.upLoadImg.imgSrc}" />
														</a>
														<div class="proinfo">
															<h2>
																<a style="margin-left:10px;color:#333333;" href="digitaldetail?digitalId=${i.digitalId}">${i.digitalName}</a>
															</h2>
															<p>
																<i>￥${i.price}</i>
																<button class="btn btn-xs" style="background:#333333; color:#fff;float:right; " onclick="addToCart(${i.digitalId})" href="javascript:void(0)" data-toggle="modal" data-target=".bs-example-modal-sm">
...
<script type="text/javascript">
	...
	$(".homePage").click(function(){
		window.location="${bsePath}DigitalList?action=list&page=1";
	})
	$(".prePage").click(function(){
		window.location="${basePath}DigitalList?action=list&page=${pageBean.prePage}";
	})
	$(".nextPage").click(function(){
		
		window.location="${basePath}DigitalList?action=list&page=${pageBean.nextPage}";
	})
	$(".lastPage").click(function(){
		window.location="${basePath}DigitalList?action=list&page=${pageBean.pageCount}";
	})
	//控制页面跳转范围
	$(".page-go").click(function(){
		...
		if(page > pageCount || page < 1 ){
			...
			window.location="${basePath}DigitalList?action=list&page="+page;
		}
	}) 
