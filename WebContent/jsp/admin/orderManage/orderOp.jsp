				...
				<c:forEach items="${orderList}" var="i">
					<tr>
						...
						<td>
							...
							<c:if test="${i.orderStatus eq 40 }"><span style="background:green;color:#fff;">退货待审批</span></c:if>
							<c:if test="${i.orderStatus eq 3 }"><span >已完成</span></c:if>
							<c:if test="${i.orderStatus eq 41 }"><span >拒绝退货</span></c:if>
							<c:if test="${i.orderStatus eq 42 }"><span >同意退货</span></c:if>
						</td>
						<td>
							...
							<c:if test="${i.orderStatus eq 40 }">
								<a class="btn btn-info btn-sm" href="jsp/admin/OrderManageServlet?action=ship&id=${i.orderId}&status=41" onclick="return confirm('确定审批通过么！')">审批通过</a>
								<a class="btn btn-info btn-sm" href="jsp/admin/OrderManageServlet?action=ship&id=${i.orderId}&status=42" onclick="return confirm('确定拒绝么！')">拒绝</a>
