...
<body>
	...
	<h2 class="text-center">图书信息管理</h2>
	<div class="container content">
		<div class="funbtn">
			<a id="batDel" class="btn btn-danger" href="javascript:void(0)">批量删除</a>
			<a class="btn btn-info" href="jsp/admin/msgManage/bookAdd.jsp" >增加图书</a>
		</div>
		<table class="table table-striped table-hover">
			...
			<c:choose>
				<c:when test="${!empty bookList}">
					<c:forEach items="${bookList}" var="i" varStatus="n">
						<tr>
							...
							<td>${i.bookName}</td>
							<td class="noClick">
								<a class="btn btn-danger btn-sm" href="jsp/admin/BookServlet?action=del&id=${i.bookId}" onclick="javascript:return confirm('确定要删除吗？');">删除</a>
							</td>
						</tr>
					</c:forEach>
				</c:when>
				<c:otherwise>
					<tr>
						<td colspan="8"><h4 class="text-center">当前无更多信息</h4></td>
					</tr>
				</c:otherwise>
			</c:choose>
		</table>
...
<script type="text/javascript">
	...
	$(".homePage").click(function(){
		window.location="${bsePath}jsp/admin/BookServlet?action=list&page=1";
	})
	$(".prePage").click(function(){
		window.location="${basePath}jsp/admin/BookServlet?action=list&page=${pageBean.prePage}";
	})
	$(".nextPage").click(function(){
		
		window.location="${basePath}jsp/admin/BookServlet?action=list&page=${pageBean.nextPage}";
	})
	$(".lastPage").click(function(){
		
		window.location="${basePath}jsp/admin/BookServlet?action=list&page=${pageBean.pageCount}";
	})
	//控制页面跳转范围
	$(".page-go").click(function(){
		...
		if(page > pageCount || page < 1 ){
			...
			window.location="${basePath}jsp/admin/BookServlet?action=list&page="+page;
		}
	})
	...
	$("#batDel").click(function(){
		...
		if(confirm("你确定要删除"+choices.length+"条用户吗？")){
			...
			window.location = "${basePath}jsp/admin/BookServlet?action=batDel&ids="+ids;
