...
<body>
	...
	<div class="container content">
		<div class="funbtn">
			...
			<a class="btn btn-info" href="MsgServlet?action=addView">增加文章</a>
			<div class="search col-md-4">
						<div class="input-group">
							<form action="MsgServlet" method="post">
								<input type="hidden" value="seach" name="action">
		     	 				<input style="float: left;width: 160px;" type="text" class="form-control" name="title" placeholder="输入要搜索的文章名...">
		       					&nbsp;&nbsp;&nbsp;
		...
		<table class="table table-striped table-hover">
			...
			<c:choose>
				<c:when  test="${!empty messageList}">
					<c:forEach items="${messageList}" var="i" varStatus="n">
						<tr>
							...
							<td>
								<a class="btn btn-default btn-sm" href="MsgServlet?action=detail&id=${i.id}">详情</a>
								<a class="btn btn-info btn-sm" href="MsgServlet?action=edit&id=${i.id}">修改</a>
								<a class="btn btn-danger btn-sm" href="MsgServlet?action=del&id=${i.id}" onclick="javascript:return confirm('确定要删除吗？');">删除</a>
							</td>
						</tr>
					</c:forEach>
				</c:when>
				<c:otherwise>
					<tr>
						<td colspan="8"><h4 class="text-center">当前无更多文章信息</h4></td>
					</tr>
				</c:otherwise>
			</c:choose>
		</table>
...
<script type="text/javascript">
	...
	$(".homePage").click(function(){
		window.location="${basePath}/jsp/admin/MsgServlet?action=list&page=1";
	})
	...
	$("#batDel").click(function(){
		...
		if(confirm("你确定要删除"+choices.length+"条文章吗？")){
			window.location="${basePath}/jsp/admin/MsgServlet?action=batDel&ids="+ids;
		}
	})
